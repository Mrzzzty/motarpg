...
function run(): void {
  ...
  for (const floorId of FLOORS) {
    for (let it = 0; it < ITER; it++) {
      ...
      if (!gridReachable(floor)) { stats.gridReachFailures++; stats.failures++; }
      ...
      // 内容规则：怪物距入口≥3格、精英只在精英房、宝箱房2-3宝箱
      for (const room of floor.rooms) {
        const monsters = room.entities.filter(e => e.kind === 'monster');
        for (const m of monsters) {
          if (m.isElite && room.type !== 'elite') { stats.eliteOutsideEliteRoom++; stats.failures++; }
        }
        if (room.type === 'chest') {
