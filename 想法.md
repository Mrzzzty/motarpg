...
> - **战斗托管/微操**：❌ 仍未实现（当前为「战前确认 + 全自动结算」）。
> - **光线追踪（2D 光线投射 / 72 条射线）**：⏭ 已被整合文档 §16 的 **Three.js 原生阴影（PCFSoftShadowMap）+ Bloom + 丁达尔光锥** 取代，不再单独实现；PixiJS 着色器方案（§15）亦作废。
...
> 表中 ✅ = 已确定，❓ = 待讨论；**楼层区间未定**（见待讨论项 1）。

| 区段（从最低到最高） | 主题风格 | 氛围 / 备注 | 状态 |
| :--- | :--- | :--- | :--- |
| 塔楼底部（最低层，比低层还低；玩家出生的一层） | **青砖世界** | 大量青砖铸成的"世界"，根基 / 牢笼意象 | ✅ 已定 |
| 低层 | 待定（候选：王城 / 废墟 / 森林…） | 塔楼底部之上的过渡段 | ❓ 待讨论 |
| 中层 | **图书馆** | 静谧、书卷气、暖棕木色 | ✅ 已定 |
| 高层 | **观星台** | 星空、冷蓝、旷远；可复用已做的星空 / 月亮背景 | ✅ 已定 |
| 顶部（塔顶前过渡段） | 待定 | 高层与塔顶之间的过渡 | ❓ 待讨论 |
| 塔顶（原以为的终点） | 待定 | 误导玩家"登顶=真相"的**假终点**（见「四、终局反转」） | ❓ 待讨论 |
| 地下段（详见「四、终局反转」） | 地下 / 地心 / 地狱 / 天堂 | 反向纵深，越下越深 | ❓ 待讨论 |
...
| 区段 | 房间外内容 | 状态 |
| :--- | :--- | :--- |
| 塔楼底部（底层，青砖世界） | 地面；其他楼房的楼顶（楼顶存疑，用户未确认是否保留） | ✅ 已定（楼顶待确认） |
| 低层 | 待定 | ❓ 待讨论 |
| 中层（图书馆） | 云雾缭绕 | ✅ 已定 |
...
1. **楼层总数与分段粒度**：塔总层数？几个区段？每段多少层？（影响主题表规模与进层切换频率）
2. **各区段主题基调**：**已定——塔楼底部 = 青砖世界、中层 = 图书馆、高层 = 观星台**；低层、塔顶假终点、地下四区的主题仍需敲定（候选见上表）。
...
6. **房间外内容（窗外 / 远景）**：**已定——塔楼底部 = 地面 + 其他楼房楼顶（楼顶存疑）、中层 = 云雾缭绕、高层 = 星光闪闪且天空顶部发紫**；低层、顶部、塔顶、地下段的窗外内容待定。

## 三、与现有代码衔接（现状 & 待办）

- **现状**：`FloorGenerator` 目前**随机分配房间类型**，无"楼层主题"概念；`ThreeRenderer` 按房间类型着色，但**没有按层切换整体美术风格**。
- **待实现（若落地）**：新增「楼层主题表」（区间 → 配色/光照/雾色/背景/房间类型权重），`GameController` 在进层时切换 `scene` 的 `environment` 与 `ThreeRenderer` 调色。已做的星空、月亮、墙体虚化可作为高层风格的素材。
