...
var Player = class _Player {
  ...
  usePotion(tier) {
    ...
    const def2 = dataManager.getPotion(tier);
    if (!def2) return false;
    this.state.potions[tier] -= 1;
    const s = this.stats();
    const pct = def2.healPct * (1 + s.potionBonus / 100);
  ...
  bestPotionFor(missing) {
    ...
    for (const tier of order) {
      const def2 = dataManager.getPotion(tier);
      if (!def2) continue;
      if (this.getPotionCount(tier) > 0 && Math.round(this.maxHp * def2.healPct) <= missing) return tier;
    }
    for (const tier of order) {
...
function eq(a, b, msg) {
  if (a === b) {
    ...
    console.error(`\u2717 ${msg}: \u671F\u671B ${b}\uFF0C\u5B9E\u9645 ${a}`);
    process.exit(1);
  }
}
dataManager.loadAll();
var rm = RelicManager.getInstance();
...
var attack0 = player.stats().attack;
...
var before15 = player.stats().attack;
rm.add("R015");
ok(player.stats().attack > before15, "R015 \u767E\u5206\u6BD4\u653B\u51FB\u751F\u6548");
var hpBefore = player.stats().maxHp;
rm.add("R004");
...
player.state.hp = 1;
var lowAtk = player.stats().attack;
player.state.hp = player.stats().maxHp;
var highAtk = player.stats().attack;
...
var normalMul = DifficultySystem.getInstance().monsterMul();
eq(normalMul, 1, "\u666E\u901A\u96BE\u5EA6\u602A\u7269\u500D\u7387 1.0");
gameState.setDifficulty(7);
eq(DifficultySystem.getInstance().monsterMul(), 5, "\u5929\u5802\u96BE\u5EA6\u602A\u7269\u500D\u7387 5.0");
eq(DifficultySystem.getInstance().startCurseCount(), 2, "\u5929\u5802\u5F00\u5C40\u707E\u5384 2 \u4EF6");
var def = dataManager.monsters.monsters[0];
var hpNormal = StatCalculator.getInstance().monsterStats(def, 10, false, 1).hp;
gameState.setDifficulty(2);
var hpN = StatCalculator.getInstance().monsterStats(def, 10, false, 1).hp;
gameState.setDifficulty(7);
var hpH = StatCalculator.getInstance().monsterStats(def, 10, false, 1).hp;
...
rm.resetRun();
player.state.relics = [];
var curses = rm.grantStartCurses(2);
...
var cands = rm.randomCandidates(3);
...
var missingIcons = rm.all.filter((d) => !dataManager.relics.icons?.[d.id]).map((d) => d.id);
ok(missingIcons.length === 0, `icons \u6620\u5C04\u8986\u76D6\u5168\u90E8\u9057\u7269\uFF08\u7F3A\uFF1A${missingIcons.join(",") || "\u65E0"}\uFF09`);
console.log(`  \xB7 \u9057\u7269\u603B\u6570 ${rm.all.length}\uFF0C\u56FE\u6807\u6620\u5C04 ${Object.keys(dataManager.relics.icons ?? {}).length} \u6761`);
gameState.setDifficulty(2);
