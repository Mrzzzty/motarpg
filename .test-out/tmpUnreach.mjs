...
// src/test/tmpUnreach.ts
dataManager.loadAll();
var gen = MapGenerator.getInstance();
for (let i = 0; i < 4e4; i++) {
  const floor = gen.generate(21);
  const { grid } = floor;
  const seen = /* @__PURE__ */ new Set([cellKey(floor.entryX, floor.entryY)]);
  ...
  for (const room of floor.rooms) {
    for (const d of room.doors) if (!seen.has(cellKey(d.x, d.y))) bad = `\u95E8 ${d.x},${d.y} \u623F${room.id}(${room.type})`;
    for (const e of room.entities) {
      if (["stair", "npc", "chest", "monster", "boss"].includes(e.kind) && !seen.has(cellKey(e.x, e.y))) {
  ...
  for (const room of floor.rooms) {
    console.log(`\u623F ${room.id} ${room.type} rect(${room.x},${room.y},${room.width}x${room.height}) \u95E8${JSON.stringify(room.doors.map((d) => `${d.x},${d.y}:${d.direction}`))} \u5B9E\u4F53${JSON.stringify(room.entities.map((e) => `${e.kind}@${e.x},${e.y}`))}`);
  }
  for (let row = 0; row < floor.height; row++) {
    ...
    for (let col = 0; col < floor.width; col++) {
      ...
      const reach = seen.has(cellKey(col, row));
      const ent = floor.rooms.flatMap((r) => r.entities).find((e) => e.x === col && e.y === row);
      line += isEntry ? "E" : t === -1 ? " " : t === 1 ? "#" : t === 2 ? "o" : t === 4 ? "X" : ent ? ent.kind === "monster" ? "m" : ent.kind === "chest" ? "c" : ent.kind === "stair" ? "s" : ent.kind[0] : reach ? "." : "~";
    }
