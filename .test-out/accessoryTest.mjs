...
var StatCalculator = class _StatCalculator {
  ...
  playerBaseAt(level) {
    const cfg = dataManager.config;
    const base2 = cfg.playerBase;
    let hp = base2.maxHp;
    let atk = base2.attack;
    let def = base2.defense;
  ...
  /** 升到下一级所需经验 */
  expToNext(level) {
    const { base: base2, power } = dataManager.config.expFormula;
    return Math.round(base2 * Math.pow(level, power));
  }
};
...
var Player = class _Player {
  ...
  state;
  constructor() {
    const base2 = dataManager.config.playerBase;
    this.state = {
      level: 1,
      exp: 0,
      hp: base2.maxHp,
      baseMaxHp: base2.maxHp,
      baseAttack: base2.attack,
      baseDefense: base2.defense,
      baseCritRate: base2.critRate,
      baseDodgeRate: base2.dodgeRate,
      gold: 0,
      keys: 0,
  ...
  checkLevelUp() {
    ...
    while (this.state.exp >= need && this.state.level < 999) {
      ...
      this.state.level += 1;
      const base2 = calc.playerBaseAt(this.state.level);
      const gainedHp = base2.maxHp - this.state.baseMaxHp;
      const gainedAtk = base2.attack - this.state.baseAttack;
      const gainedDef = base2.defense - this.state.baseDefense;
      this.state.baseMaxHp = base2.maxHp;
      this.state.baseAttack = base2.attack;
      this.state.baseDefense = base2.defense;
...
var EquipmentGenerator = class _EquipmentGenerator {
  ...
  generate(source, opts = {}) {
    ...
    const floorId = opts.floorId ?? player2.state.currentFloor;
    ...
    const level = this.rollEquipLevel(player2.state.level, floorId);
...
dataManager.loadAll();
var gen = EquipmentGenerator.getInstance();
...
var slotCount = { weapon: 0, armor: 0, accessory: 0 };
...
console.log("=== \u6570\u503C\u6210\u957F ===");
function avgValue(quality, level, n = 200) {
  let sum = 0;
  for (let i = 0; i < n; i++) {
...
var poor = avgValue("poor", 3);
var common = avgValue("common", 3);
var fine = avgValue("fine", 3);
var rare = avgValue("rare", 3);
...
var dodgeAcc = { ...critAcc, id: "test_acc_dodge", accessoryStat: "dodge", accessoryValue: 3.2 };
player.addEquipment(critAcc);
player.addEquipment(dodgeAcc);
var beforeStats = player.stats();
check(
  "\u672A\u88C5\u5907\u9970\u54C1\u65F6\u4E3A\u57FA\u7840\u503C",
  beforeStats.critRate === 5 && beforeStats.dodgeRate === 3,
  `crit=${beforeStats.critRate} dodge=${beforeStats.dodgeRate}`
);
player.equip(critAcc.id);
var withCrit = player.stats();
