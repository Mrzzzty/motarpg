...
var StatCalculator = class _StatCalculator {
  ...
  monsterStats(def2, floorId, isElite, depth = 1) {
    ...
    let hp = this.anchorValue(a.hp, floorId, a.overflowPerFloor.hp) * def2.hpMul * depthMul;
    let atk = this.anchorValue(a.atk, floorId, a.overflowPerFloor.atk) * def2.atkMul * depthMul;
    let defv = this.anchorValue(a.def, floorId, a.overflowPerFloor.def) * def2.defMul * depthMul;
    let exp = this.anchorValue(a.exp, floorId, a.overflowPerFloor.exp) * def2.expMul * depthMul;
    let gold = this.anchorValue(a.gold, floorId, a.overflowPerFloor.gold) * def2.goldMul * depthMul;
    if (isBoss) {
