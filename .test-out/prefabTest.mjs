    ...
    for (let x = 0; x < width; x++) {
      ...
      switch (ch) {
        ...
        case "3": {
          ...
          const def2 = rng.pickWeighted(monsterPool, (m) => m.weight);
          entities.push({
...
function buildPrefabFloor(def2) {
  const { grid, entities } = parseRows(def2.rows, def2.floorId);
  const width = grid[0]?.length ?? 0;
  const height = grid.length;
  let entry = def2.entry ?? null;
  if (!entry) {
    for (let y = 0; y < height && !entry; y++) {
      for (let x = 0; x < width && !entry; x++) {
        if ((def2.rows[y]?.[x] ?? "1") === "S") entry = { x, y };
      }
    }
  }
  const rooms = buildRooms(def2.rooms, def2.floorId, entities, grid);
  if (rooms.length === 0) throw new Error(`[Prefab] \u697C\u5C42${def2.floorId} \u672A\u5B9A\u4E49\u4EFB\u4F55\u623F\u95F4`);
  const entryPoint = [entry, rooms[0] ? { x: rooms[0].centerX, y: rooms[0].centerY } : null].find((p) => !!p && grid[p.y]?.[p.x] === 0);
  if (!entryPoint) throw new Error(`[Prefab] \u697C\u5C42${def2.floorId} \u5165\u53E3\u65E0\u6548\uFF08\u5FC5\u987B\u662F\u7A7A\u5730\uFF09`);
  for (const e of entities) {
    ...
    Logger.warn(`[Prefab] \u697C\u5C42${def2.floorId} \u5B9E\u4F53 ${e.id} (${e.x},${e.y}) \u5728\u6240\u6709\u623F\u95F4\u5916\uFF0C\u6302\u5230 ${nearest.id}`);
  ...
  return {
    floorId: def2.floorId,
    kind: def2.kind ?? "normal",
    rooms,
    corridors: [],
  ...
  chooseCell(px, py, occupied, dirHistory, bias) {
    ...
    const tryPick = (allowReverse, allowStraight) => {
      ...
      for (const dir of shuffled) {
        if (!allowReverse && lastDir && dir === OPPOSITE2[lastDir]) continue;
...
var StatCalculator = class _StatCalculator {
   ...
   */
  monsterStats(def2, floorId, isElite, depth = 1) {
    const cfg = dataManager.config;
    const a = this.anchors;
    const round = (v) => Math.max(1, Math.round(v));
    const isBoss = def2.category === "boss";
    const depthMul = this.depthMultiplier(floorId, depth);
    const diff = DifficultySystem.getInstance();
    const dmul = diff.monsterMul();
    let hp = this.anchorValue(a.hp, floorId, a.overflowPerFloor.hp) * def2.hpMul * depthMul * dmul;
    let atk = this.anchorValue(a.atk, floorId, a.overflowPerFloor.atk) * def2.atkMul * depthMul * dmul;
    let defv = this.anchorValue(a.def, floorId, a.overflowPerFloor.def) * def2.defMul * depthMul * dmul;
    let exp = this.anchorValue(a.exp, floorId, a.overflowPerFloor.exp) * def2.expMul * depthMul * diff.expMul();
    let gold = this.anchorValue(a.gold, floorId, a.overflowPerFloor.gold) * def2.goldMul * depthMul * diff.goldMul();
    if (isBoss) {
    ...
    return {
      name: isElite ? `\u7CBE\u82F1\xB7${def2.name}` : def2.name,
      hp: round(hp),
      attack: round(atk),
      defense: round(defv),
      exp: round(exp),
  ...
  playerBaseAt(level) {
    ...
    return { maxHp: hp, attack: atk, defense: def2 };
  }
  /** 升到下一级所需经验 */
  expToNext(level) {
...
var RoomFill = class {
  ...
  monsterAt(id, isElite, p) {
    const def2 = dataManager.getMonster(id);
    const stats = def2 ? StatCalculator.getInstance().monsterStats(def2, this.floorId, isElite, this.room.depth) : void 0;
  ...
  bossAt(id, p) {
    const def2 = dataManager.getMonster(id);
    const stats = def2 ? StatCalculator.getInstance().monsterStats(def2, this.floorId, false, this.room.depth) : void 0;
  ...
  validate(roomType, pool, densityCap = Infinity) {
    ...
    const monsterCount2 = () => this.room.entities.filter((e) => e.kind === "monster" || e.kind === "boss").length;
    ...
    for (const stair of this.room.entities.filter((e) => e.kind === "stair")) {
      ...
      if (!guarded && pool.length > 0 && monsterCount2() < densityCap) {
    ...
    if (roomType !== "boss") {
      ...
      while (unguarded.length > 0 && pool.length > 0 && monsterCount2() < densityCap) {
      ...
      if (unguarded.length > 0) {
        issues.push(monsterCount2() >= densityCap ? `\u5B9D\u7BB1\u65E0\u5B88\u62A4\uFF08\u5BC6\u5EA6\u4E0A\u9650\u5DF2\u6EE1 ${monsterCount2()}/${densityCap}\uFF0C\u672A\u5B88\u62A4 ${unguarded.length} \u5EA7\uFF09` : "\u5B9D\u7BB1\u65E0\u5B88\u62A4\uFF08\u65E0\u53EF\u653E\u7F6E\u683C\uFF09");
      }
    }
    if (roomType === "boss" && pool.length > 0) {
      if (this.bypassRoute() && monsterCount2() === 0) {
      ...
      while (route && added < 12) {
        const spot = monsterCount2() < densityCap ? this.freeSpotOnRoute(route) : null;
        if (spot) {
  ...
  upgradeToElite(e) {
    e.isElite = true;
    const def2 = dataManager.getMonster(e.monsterId ?? "");
    if (def2) e.stats = StatCalculator.getInstance().monsterStats(def2, this.floorId, true, this.room.depth);
  }
  /**
...
var ContentFiller = class _ContentFiller {
  ...
  convergeFloorStats(rooms, floorId) {
    ...
    for (const e of monsters.filter((m) => m.isElite)) {
      const def2 = dataManager.getMonster(e.monsterId ?? "");
      if (!def2) continue;
      const base = statCalc.monsterStats(def2, floorId, false, 1);
...
var MapGenerator = class _MapGenerator {
  ...
  generate(floorId) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        const floor2 = this.tryGenerate(floorId);
        if (floor2) {
  ...
  describe(floor2) {
    const lines = [];
    lines.push(`=== \u697C\u5C42 ${floor2.floorId}\uFF08${floor2.kind}\uFF09 ${floor2.rooms.length}\u4E2A\u623F\u95F4 ${floor2.corridors.length}\u6761\u8D70\u5ECA ===`);
    for (const room of this.roomsSorted(floor2)) {
    ...
    lines.push(`\u5165\u53E3: (${floor2.entryX},${floor2.entryY}) \u8DEF\u5F84\u6570: \u89C1\u9A8C\u8BC1\u5668`);
    return lines.join("\n");
  }
  /**
  ...
  roomsSorted(floor2) {
    return [...floor2.rooms].sort((a, b) => a.order - b.order);
  }
};

// src/test/prefabTest.ts
var def = {
  floorId: 1,
  kind: "normal",
  rooms: [
...
dataManager.loadAll();
var floor = buildPrefabFloor(def);
var failed = 0;
var check = (name, ok, detail = "") => {
  console.log(`${ok ? "\u2714" : "\u2718"} ${name}${ok ? "" : ` \u2014\u2014 ${detail}`}`);
  if (!ok) failed++;
};
var ids = (t) => floor.rooms.filter((r) => r.type === t).map((r) => r.id);
check("\u7F51\u683C\u5C3A\u5BF8 25\xD712", floor.width === 25 && floor.height === 12);
...
check(
  ...
  !floor.connections.some((c) => c.from === ids("combat")[0] && c.to === ids("chest")[0] || c.from === ids("chest")[0] && c.to === ids("combat")[0])
);
var monsterCount = floor.rooms.flatMap((r) => r.entities).filter((e) => e.kind === "monster" && !e.isElite).length;
check("\u654C\u4EBA\u5DF2\u653E\u7F6E\uFF08monster=1\uFF09", monsterCount === 1, `\u5B9E\u9645 ${monsterCount}`);
var chest = floor.rooms.flatMap((r) => r.entities).find((e) => e.kind === "chest");
check("\u5B9D\u7BB1\u5F52\u5C5E\u5B9D\u7269\u95F4", !!chest && chest.x === 12 && chest.y === 8);
var merchant = floor.rooms.flatMap((r) => r.entities).find((e) => e.kind === "npc");
