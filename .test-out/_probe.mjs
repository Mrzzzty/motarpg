...
for (const f of [55, 41, 33, 27]) {
  for (let it = 0; it < 40; it++) {
    const floor = gen.generate(f);
    const mons = floor.rooms.flatMap((r) => r.entities.filter((e) => e.kind === "monster" && e.stats).map((e) => ({ room: r, isElite: !!e.isElite, def: e.monsterId ?? "", stats: e.stats, layout: r.layout })));
    ...
    let bad = false;
    for (const attr of attrs) {
      const min = Math.min(...pool.map((m) => m.stats[attr]));
      const max = Math.max(...pool.map((m) => m.stats[attr]));
      if (max > min * 1.4 + 1e-9) {
        bad = true;
        console.log(`[\u8FDD\u89C4] floor${f} it${it} ${attr} ${min}~${max} ratio=${(max / min).toFixed(3)}`);
        for (const m of pool) {
          console.log(`   ${m.def} elite=${m.isElite} depth=${m.room.depth} type=${m.room.type} layout=${m.layout} ${attr}=${m.stats[attr]} hp=${m.stats.hp} atk=${m.stats.attack}`);
        }
      }
    }
    if (bad) {
...
console.log("\u672A\u590D\u73B0\uFF0840 \u6B21 \xD7 4 \u5C42\u5185\u5747\u6B63\u5E38\uFF09");
