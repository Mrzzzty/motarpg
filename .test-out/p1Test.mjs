...
var StatCalculator = class _StatCalculator {
  ...
  anchorValue(series, floorId, overflowRate) {
    const floors2 = this.anchors.floors;
    const last = floors2.length - 1;
    if (floorId <= floors2[0]) return series[0];
    if (floorId >= floors2[last]) {
      const over = floorId - floors2[last];
      return series[last] * Math.pow(1 + overflowRate, over);
    }
    for (let i = 0; i < last; i++) {
      if (floorId >= floors2[i] && floorId <= floors2[i + 1]) {
        const t = (floorId - floors2[i]) / (floors2[i + 1] - floors2[i]);
...
var floors = [3, 6, 8, 11, 14, 16, 19, 21, 24, 27, 31, 36, 42, 47, 53, 5, 10, 15, 20, 25, 40];
var ITER = 40;
var hiddenSeen = 0;
var restRoomsSeen = 0;
var chestRoomsChecked = 0;
var bossLayouts = {};
for (const f of floors) {
  for (let it = 0; it < ITER; it++) {
    ...
    for (const h of hidden) {
      ...
      check("\u9690\u85CF\u623F\u5B9E\u4F53=\u7CBE\u82F1+\u5927\u5B9D\u7BB1", h.entities.filter((e) => e.kind === "monster" && e.isElite).length === 1 && h.entities.filter((e) => e.kind === "chest" && e.chestTier === "grand").length === 1, h.id);
      ...
      const seen = /* @__PURE__ */ new Set([`${ent.x},${ent.y}`]);
    ...
    if (floor.kind === "normal") {
      ...
      for (const railKey of ["onPathA", "onPathB"]) {
    ...
    for (const room of floor.rooms) {
      if (room.type === "chest") {
        ...
        for (const chest of room.entities.filter((e) => e.kind === "chest")) {
          const guarded = room.entities.some((e) => (e.kind === "monster" || e.kind === "boss") && Math.abs(e.x - chest.x) + Math.abs(e.y - chest.y) <= 2);
      ...
      if (room.type === "end" && floor.kind === "normal") {
        ...
        if (stair) {
          const guarded = room.entities.some((e) => (e.kind === "monster" || e.kind === "boss") && Math.abs(e.x - stair.x) + Math.abs(e.y - stair.y) <= 3);
...
var gauntletSeen = (bossLayouts["boss_gauntlet"] ?? 0) > 0;
var arenaSeen = (bossLayouts["boss_arena"] ?? 0) > 0;
