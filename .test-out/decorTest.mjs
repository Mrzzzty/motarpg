...
var RoomGenerator = class _RoomGenerator {
  ...
  place(floorId, plan) {
    ...
    const occupied2 = /* @__PURE__ */ new Map();
    const rooms = [];
    const dirHistory = [];
    ...
    for (let i = 0; i < plan.length; i++) {
      ...
      if (i === 0) {
        ...
        const choice = this.chooseCell(prev.gx, prev.gy, occupied2, dirHistory, target);
      ...
      const key = `${gx},${gy}`;
      if (occupied2.has(key)) return null;
      occupied2.set(key, i);
   ...
   */
  chooseCell(px, py, occupied2, dirHistory, bias) {
    const cfg = dataManager.mapGen;
    const R = cfg.gridRadius;
    ...
    const tryPick = (allowReverse, allowStraight) => {
      ...
      for (const dir of shuffled) {
        ...
        const gx = px + dx;
        const gy = py + dy;
        if (occupied2.has(`${gx},${gy}`)) continue;
    ...
    return tryPick(false, false) ?? tryPick(true, false) ?? tryPick(true, true) ?? this.nearestFreeCell(px, py, occupied2, R);
  }
  /** 第3次连续同向判定 */
  ...
  nearestFreeCell(px, py, occupied2, R) {
    let best = null;
    let bestD = Infinity;
...
function decorBaseY(kind) {
  return kind === "chandelier" ? 2.1 : 0;
}
var ON_WALL = /* @__PURE__ */ new Set(["banner", "sconce", "web", "wallCrack"]);
var ON_FLOOR = /* @__PURE__ */ new Set(["puddle", "mossPatch", "floorCrack", "rune"]);
...
function planRoomDecor(room, grid, floorId, tier, density = 1) {
  if (density <= 0) return [];
  ...
  const solidCount = solidPool.length === 0 ? 0 : Math.max(1, Math.min(5, Math.round(perimeter / 9 * density)));
  ...
  if (room.width >= 5 && room.height >= 5 && wallPool.length > 0) {
    ...
    for (let wy = room.y; wy < room.y + room.height && placed < count; wy++) {
      for (let wx = room.x; wx < room.x + room.width && placed < count; wx++) {
        const onRing = wx === room.x || wx === room.x + room.width - 1 || wy === room.y || wy === room.y + room.height - 1;
...
function run() {
    ...
    for (let it = 0; it < ITER; it++) {
      ...
      for (const room of floor.rooms) {
        ...
        for (const item of a) {
          ...
          if (item.cellX === void 0 && doors.has(`${cx},${cy}`)) doorConflicts++;
  ...
  console.log(`\u623F\u95F4 ${rooms} \xB7 \u9648\u8BBE ${items} \u4EF6 \xB7 \u5E73\u5747 ${(items / Math.max(1, rooms)).toFixed(1)} \u4EF6/\u623F`);
