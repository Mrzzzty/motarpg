  ...
  for (const floorId of FLOORS) {
    for (let it = 0; it < ITER; it++) {
      ...
      if (RELIC_CHEST_FLOORS.includes(floorId)) {
        ...
        if (!has) {
          relicChestMissing++;
          console.log(`[\u9057\u7269\u5B9D\u7BB1\u7F3A\u5931] \u697C\u5C42${floorId}`);
        }
      }
      const availNormals = dataManager.monsters.monsters.filter((m) => m.category === "normal" && floorId >= m.floorMin && floorId <= m.floorMax);
